...
public class ProductRegisterAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	super.setRedirect(false);
			...
			// spec 목록을 보여주고자 한다.
	         List<String> specList = pdao.selectSpecList();
		...
		*/
			...
			// === 첨부되어지는 이미지 파일이 올라가는 절대경로 imagesDir ==> C:\myjsp\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\MyMVC\images 
		...
		// === 첨부 이미지 파일을 올렸으니 그 다음으로 제품정보를 (제품명, 정가, 제품수량,...) DB의 shopping_product 테이블에 insert 를 해주어야 한다.  === 
		...
		*/
		...
		// *** 크로스 사이트 스크립트 공격에 대응하는 안전한 코드(시큐어 코드) 작성하기 *** //
		  pcontent =  MyUtil.replaceParameter(pcontent);
		  ...
		  int n = pdao.productInsert(pvo);   // shopping_product 테이블에 insert 하기
		  ...
		  if(n*m==1) {
			  message = "제품등록 성공!!";
			  loc = request.getContextPath()+"/shop/mallHome.up";
		  }
		  else {
			  message = "제품등록 실패!!";
