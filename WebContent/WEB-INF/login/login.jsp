...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		// === 로컬 스토리지(localStorage)에 저장된 userid 값을 불러와서 input 태그 userid에 넣어주기 === //
		var loginUserid = localStorage.getItem('saveid');
		
		if(loginUserid != null) {
			
			$("#loginUserid").val(loginUserid);
			$("input:checkbox[id=saveid]").prop("checked", true); // 아이디를 넣어줌과 동시에 체크박스에도 체크되게 함
		}
	...
	*/
	...
	if( $("input:checkbox[id=saveid]").prop("checked") ) {
		...
		// alert("아이디 저장 체크 안됨");
		localStorage.removeItem('saveid'); // 아이디 저장 체크를 해제한 경우 저장된 아이디를 없애라는 뜻
	}
	...
	function goCoinPurchaseTypeChoice(idx) {
		...
		// 코인구매 금액 선택 팝업창 띄우기
		var url = "<%= request.getContextPath()%>/member/coinPurchaseTypeChoice.up?idx="+idx;
		
		window.open(url, "coinPurchaseTypeChoice", 
					"left=350px, top=100px, width=650px, height=570px"); 
		
	}// end of function goCoinPurchaseTypeChoice(idx)------------------
	
	
	function goCoinPurchaseEnd(idx, coinmoney) {
	//	alert("확인용 idx : "+idx+", coinmoney : "+coinmoney);
		
	// 아임포트(결제관련 외부 API) 결제 팝업창 띄우기
		var url="<%= request.getContextPath()%>/member/coinPurchaseEnd.up?idx="+idx+"&coinmoney="+coinmoney;
